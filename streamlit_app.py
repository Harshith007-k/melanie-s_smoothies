...
if not st.session_state.authenticated:
    ...
    if not bookings_df.empty:
        ...
        # Select Booking to Update or Delete
        booking_to_manage = st.selectbox("Select Booking to Manage", bookings_df["User"].unique())

        selected_booking = bookings_df[bookings_df["User"] == booking_to_manage].iloc[0]
        ...
        with st.form("update_booking_form"):
            ...
            for _, booking in bookings_df[(bookings_df["Date"] == pd.Timestamp(updated_date)) & (bookings_df["Room"] == updated_room)].iterrows():
                if (updated_start_datetime < booking["End"]) and (updated_end_datetime > booking["Start"]) and booking["User"] != booking_to_manage:
                    conflict = True
            ...
            if submit_button and not conflict:
                bookings_df.loc[bookings_df["User"] == booking_to_manage, ["User", "Email", "Room", "Priority", "Description", "Date", "Start", "End"]] = [
                ...
                # Save updated bookings (to file or database)
                # save_bookings(bookings_df)
        ...
        delete_button = st.button("Delete Booking")
        if delete_button:
            bookings_df = bookings_df[bookings_df["User"] != booking_to_manage]
            # Save the updated bookings (to file or database)
            # save_bookings(bookings_df)
            st.success(f"🚨 Booking for {booking_to_manage} has been deleted.")
